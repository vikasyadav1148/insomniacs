 ...
 * Description: Premium, gold-accented high-fidelity single actor profile template.
 * Author: Vikas Yadav
 */
...
// Fallback Unyson helper functions
...
add_action( 'admin_init', 'insom_actor_settings_register' );
    ...
    if ( $current_term && ! is_wp_error( $current_term ) ) {
        ...
        $actor_bio = get_term_meta( $term_id, 'insom_actor_bio', true ) ?: $current_term->description;
        if ( empty( $actor_bio ) ) {
        ...
        $actor_image = blockter_cache_external_image(get_term_meta( $term_id, 'insom_actor_image', true ));
        if ( empty( $actor_image ) ) {
            $actor_image = blockter_cache_external_image('https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80&w=400');
        }
    ...
    // VALIDATION: Force recalculation if years_active starts before birth date or seems garbage (pre-1960)
    $birth_year = 0;
    if (!empty($birth_date) && preg_match('/(\d{4})/', $birth_date, $b_matches)) {
        $birth_year = (int)$b_matches[1];
    }
    
    // If years_active is invalid (starts too early) or empty, force reset
    $should_recalculate = false;
    if (empty($years_active)) {
        $should_recalculate = true;
    } elseif (preg_match('/^(\d{4})/', $years_active, $y_matches)) {
        $start_year = (int)$y_matches[1];
        if (($birth_year > 1900 && $start_year < $birth_year) || ($start_year < 1960)) {
            $should_recalculate = true;
        }
    }

    if ($should_recalculate) {
        $years_active = !empty($api_data['years_active']) ? $api_data['years_active'] : ($birth_year > 1900 ? ($birth_year . ' – Present') : '1998 – Present');
    }
        ...
        $nickname = get_term_meta( $term_id, 'insom_actor_nickname', true );
        ...
        $actor_bio = get_option( 'insom_actor_bio' ) ?: esc_html($actor_name) . ' is a globally celebrated professional in the entertainment industry, widely known for their stellar performances, range, and profound dedication to bringing complex characters to life.';
        $actor_image = blockter_cache_external_image(get_option( 'insom_actor_image' ) ?: 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80&w=400');
    ...
    // Dynamic fallbacks
    if (empty($birth_date)) $birth_date = 'August 1, 1979';
    if (empty($height)) $height = '6\'4" (1.93 m)';
    if (empty($nationality)) $nationality = 'American';
    if (empty($years_active)) $years_active = '1999 – Present';
    if (empty($occupation)) $occupation = 'Actor | Producer';
    ...
    if (empty($nickname)) { $nickname = (strcasecmp($actor_name, 'JASON MOMOA') === 0) ? 'Mo, Chief' : 'N/A'; }
    if (empty($children)) { $children = 'N/A'; }
    if (empty($partner))  { $partner  = 'N/A'; }
} else {
    $actor_name = "JASON MOMOA";
    $actor_bio = "Joseph Jason Namakaeha Momoa is an American actor and producer.";
    $actor_image = "https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80&w=400";
    $current_term = null;
}

// ---------------------------------------------------------
// BACKEND ADMIN SETUP & OPTION PAGES
// ---------------------------------------------------------
...
if ( ! $is_standalone ) {
    if ( ! function_exists( 'insom_mv_actor_edit_form_fields' ) ) {
        function insom_mv_actor_edit_form_fields( $term, $taxonomy ) {
            ...
            ?>
            <tr class="form-field">
                <th scope="row" valign="top"><label>Profile Image URL</label></th>
                <td>
                    <input type="text" name="insom_actor_image" value="<?php echo esc_url( $actor_image ); ?>" style="width: 100%; max-width: 500px;" />
                </td>
            </tr>
            <tr class="form-field">
                <th scope="row" valign="top"><label>Birth Date</label></th>
                <td>
                    <input type="text" name="insom_actor_birth_date" value="<?php echo esc_attr( $actor_birth_date ); ?>" placeholder="e.g. August 1, 1979" style="width: 100%; max-width: 500px;" />
                </td>
            </tr>
            <tr class="form-field">
                <th scope="row" valign="top"><label>Height</label></th>
                <td>
                    <input type="text" name="insom_actor_height" value="<?php echo esc_attr( $actor_height ); ?>" placeholder="e.g. 1.93 m" style="width: 100%; max-width: 500px;" />
                </td>
            </tr>
            <tr class="form-field">
                <th scope="row" valign="top"><label>Nationality</label></th>
                <td>
                    <input type="text" name="insom_actor_nationality" value="<?php echo esc_attr( $actor_nationality ); ?>" placeholder="e.g. American" style="width: 100%; max-width: 500px;" />
                </td>
            </tr>
            <tr class="form-field">
                <th scope="row" valign="top"><label>Years Active</label></th>
                <td>
                    <input type="text" name="insom_actor_years_active" value="<?php echo esc_attr( $actor_years_active ); ?>" placeholder="e.g. 1998–present" style="width: 100%; max-width: 500px;" />
                </td>
            </tr>
            <tr class="form-field">
                <th scope="row" valign="top"><label>Occupation</label></th>
                <td>
                    <input type="text" name="insom_actor_occupation" value="<?php echo esc_attr( $actor_occupation ); ?>" placeholder="e.g. Actor, Producer" style="width: 100%; max-width: 500px;" />
                </td>
            </tr>
            <?php
        }
    ...
    if ( ! function_exists( 'insom_mv_actor_add_form_fields' ) ) {
        function insom_mv_actor_add_form_fields( $taxonomy ) {
            ...
            <div class="form-field">
                ...
                <input type="text" name="insom_actor_image" style="width: 100%;" />
            </div>
            <div class="form-field">
                <label>Birth Date</label>
                <input type="text" name="insom_actor_birth_date" placeholder="e.g. August 1, 1979" style="width: 100%;" />
            </div>
            <?php
        }
    }
...
if ( ! function_exists( 'insom_actor_settings_layout' ) ) {
    function insom_actor_settings_layout() {
        ...
        if ( isset($_GET['settings-updated']) && $_GET['settings-updated'] ) {
            echo '<div class="notice notice-success is-dismissible"><p><strong>Cast Profile saved and synchronized.</strong></p></div>';
        }
        ?>
        <div class="wrap" style="background: #0d0e12; color: #f1f1f1; padding: 25px; border-radius: 12px; max-width: 1000px; margin-top: 20px; font-family: sans-serif;">
            <h1 style="color: #fff; font-size: 24px; margin-bottom: 20px;">👤 Actor Profile Settings</h1>
            <form method="post" action="options.php" style="background: #15171e; padding: 25px; border-radius: 10px; border: 1px solid #232731;">
                ...
                <table class="form-table" style="width: 100%;">
                    <tr>
                        <th><label style="color:#fff;">Actor Name</label></th>
                        <td><input type="text" name="insom_actor_name" value="<?php echo esc_attr($actor_name); ?>" style="width: 100%; max-width: 400px; background:#0d0e12; border:1px solid #2d303b; color:#fff; padding:6px;" /></td>
                    </tr>
                    <tr>
                        <th><label style="color:#fff;">Bio</label></th>
                        <td><textarea name="insom_actor_bio" rows="4" style="width: 100%; max-width: 400px; background:#0d0e12; border:1px solid #2d303b; color:#fff; padding:6px;"><?php echo esc_textarea($actor_bio); ?></textarea></td>
                    </tr>
                    <tr>
                        <th><label style="color:#fff;">Profile Image URL</label></th>
                        <td><input type="text" name="insom_actor_image" value="<?php echo esc_url($actor_image); ?>" style="width: 100%; max-width: 400px; background:#0d0e12; border:1px solid #2d303b; color:#fff; padding:6px;" /></td>
                    </tr>
                    <tr>
                        <th><label style="color:#fff;">Birth Date</label></th>
                        <td><input type="text" name="insom_actor_birth_date" value="<?php echo esc_attr($actor_birth_date); ?>" style="width: 100%; max-width: 400px; background:#0d0e12; border:1px solid #2d303b; color:#fff; padding:6px;" /></td>
                    </tr>
                    <tr>
                        <th><label style="color:#fff;">Height</label></th>
                        <td><input type="text" name="insom_actor_height" value="<?php echo esc_attr($actor_height); ?>" style="width: 100%; max-width: 400px; background:#0d0e12; border:1px solid #2d303b; color:#fff; padding:6px;" /></td>
                    </tr>
                    <tr>
                        <th><label style="color:#fff;">Nationality</label></th>
                        <td><input type="text" name="insom_actor_nationality" value="<?php echo esc_attr($actor_nationality); ?>" style="width: 100%; max-width: 400px; background:#0d0e12; border:1px solid #2d303b; color:#fff; padding:6px;" /></td>
                    </tr>
                    <tr>
                        <th><label style="color:#fff;">Years Active</label></th>
                        <td><input type="text" name="insom_actor_years_active" value="<?php echo esc_attr($actor_years_active); ?>" style="width: 100%; max-width: 400px; background:#0d0e12; border:1px solid #2d303b; color:#fff; padding:6px;" /></td>
                    </tr>
                    <tr>
                        <th><label style="color:#fff;">Occupation</label></th>
                        <td><input type="text" name="insom_actor_occupation" value="<?php echo esc_attr($actor_occupation); ?>" style="width: 100%; max-width: 400px; background:#0d0e12; border:1px solid #2d303b; color:#fff; padding:6px;" /></td>
                    </tr>
                    <tr>
                        <th><label style="color:#fff;">TMS Enabled</label></th>
                        <td>
                            <select name="insom_tms_enabled" style="background:#0d0e12; border:1px solid #2d303b; color:#fff; padding:6px;">
                    ...
                    <tr>
                        <th><label style="color:#fff;">TMS API Key (Gracenote)</label></th>
                        <td><input type="password" name="insom_tms_api_key" value="<?php echo esc_attr($tms_api_key); ?>" style="width: 100%; max-width: 400px; background:#0d0e12; border:1px solid #2d303b; color:#fff; padding:6px;" /></td>
                    </tr>
                </table>
...
// ---------------------------------------------------------
// FRONTEND WEB INTERFACE (GOLD DESIGN ACCORDING TO USER SPEC)
// ---------------------------------------------------------
if ( ! $is_standalone && ! did_action( 'template_redirect' ) ) {
    return;
}

// Fetch variables & overrides
$actor_birth_date = '';
$actor_height = '';
$actor_nationality = '';
$actor_years_active = '';
...
if ( $current_term && ! is_wp_error( $current_term ) ) {
    ...
    // Core Database Persistence / Cache-Through layer
    $force_sync = isset($_GET['force_sync']) || isset($_GET['sync_fresh']);

    $db_bio = get_term_meta( $term_id, 'insom_actor_bio', true );
    if ( (empty( $db_bio ) || $force_sync) && ! empty($api_data['bio']) ) {
        update_term_meta( $term_id, 'insom_actor_bio', sanitize_textarea_field( $api_data['bio'] ) );
        if ( empty( $current_term->description ) || $force_sync ) {
            wp_update_term( $term_id, 'mv_actor', array( 'description' => $api_data['bio'] ) );
        }
    }

    $db_image = get_term_meta( $term_id, 'insom_actor_image', true );
    if ( (empty( $db_image ) || $force_sync) && ! empty($api_data['image']) ) {
        update_term_meta( $term_id, 'insom_actor_image', esc_url_raw( $api_data['image'] ) );
    }

    $db_birth = get_term_meta( $term_id, 'insom_actor_birth_date', true );
    if ( (empty( $db_birth ) || $force_sync) && ! empty($api_data['birth_date']) ) {
        update_term_meta( $term_id, 'insom_actor_birth_date', sanitize_text_field( $api_data['birth_date'] ) );
    }

    $db_height = get_term_meta( $term_id, 'insom_actor_height', true );
    if ( (empty( $db_height ) || $force_sync) && ! empty($api_data['height']) ) {
        update_term_meta( $term_id, 'insom_actor_height', sanitize_text_field( $api_data['height'] ) );
    }

    $db_nation = get_term_meta( $term_id, 'insom_actor_nationality', true );
    if ( (empty( $db_nation ) || $force_sync) && ! empty($api_data['place_of_birth']) ) {
        update_term_meta( $term_id, 'insom_actor_nationality', sanitize_text_field( $api_data['place_of_birth'] ) );
    }

    $db_years = get_term_meta( $term_id, 'insom_actor_years_active', true );
    if ( (empty( $db_years ) || $force_sync) && ! empty($api_data['years_active']) ) {
        update_term_meta( $term_id, 'insom_actor_years_active', sanitize_text_field( $api_data['years_active'] ) );
    }

    $db_occ = get_term_meta( $term_id, 'insom_actor_occupation', true );
    if ( (empty( $db_occ ) || $force_sync) && ! empty($api_data['occupation']) ) {
        update_term_meta( $term_id, 'insom_actor_occupation', sanitize_text_field( $api_data['occupation'] ) );
    }

    $db_nick = get_term_meta( $term_id, 'insom_actor_nickname', true );
    if ( (empty( $db_nick ) || $force_sync) && ! empty($api_data['nickname']) ) {
        update_term_meta( $term_id, 'insom_actor_nickname', sanitize_text_field( $api_data['nickname'] ) );
    }

    $db_child = get_term_meta( $term_id, 'insom_actor_children', true );
    if ( (empty( $db_child ) || $force_sync) && ! empty($api_data['children']) ) {
        update_term_meta( $term_id, 'insom_actor_children', sanitize_text_field( $api_data['children'] ) );
    }

    $db_partner = get_term_meta( $term_id, 'insom_actor_partner', true );
    if ( (empty( $db_partner ) || $force_sync) && ! empty($api_data['partner']) ) {
        update_term_meta( $term_id, 'insom_actor_partner', sanitize_text_field( $api_data['partner'] ) );
    }

    // Social accounts
    $db_ig = get_term_meta( $term_id, 'insom_actor_social_ig', true );
    if ( (empty( $db_ig ) || $force_sync) && ! empty($api_data['social_ig']) ) {
        update_term_meta( $term_id, 'insom_actor_social_ig', esc_url_raw( $api_data['social_ig'] ) );
    }
    $db_fb = get_term_meta( $term_id, 'insom_actor_social_fb', true );
    if ( (empty( $db_fb ) || $force_sync) && ! empty($api_data['social_fb']) ) {
        update_term_meta( $term_id, 'insom_actor_social_fb', esc_url_raw( $api_data['social_fb'] ) );
    }
    $db_tw = get_term_meta( $term_id, 'insom_actor_social_twitter', true );
    if ( (empty( $db_tw ) || $force_sync) && ! empty($api_data['social_twitter']) ) {
        update_term_meta( $term_id, 'insom_actor_social_twitter', esc_url_raw( $api_data['social_twitter'] ) );
    }
    $db_imdb = get_term_meta( $term_id, 'insom_actor_imdb_url', true );
    if ( (empty( $db_imdb ) || $force_sync) && ! empty($api_data['imdb_url']) ) {
        update_term_meta( $term_id, 'insom_actor_imdb_url', esc_url_raw( $api_data['imdb_url'] ) );
    }

    // Custom sections
    $db_awards = get_term_meta( $term_id, 'insom_actor_awards_list', true );
    if ( empty( $db_awards ) || $force_sync ) {
        if ( ! empty($api_data['awards_list']) ) {
            $clean_awards = [];
            $current_year = (int)date('Y');
            foreach ($api_data['awards_list'] as $aw) {
                $yr = isset($aw['year']) ? (int)$aw['year'] : 0;
                if ($yr > 0 && $yr > $current_year) {
                    $aw['year'] = (string)$current_year;
                }
                $clean_awards[] = $aw;
            }
            update_term_meta( $term_id, 'insom_actor_awards_list', json_encode($clean_awards, JSON_UNESCAPED_SLASHES) );
        }
    }

    $db_timeline = get_term_meta( $term_id, 'insom_actor_timeline', true );
    if ( (empty( $db_timeline ) || $force_sync) && ! empty($api_data['timeline_list']) ) {
        update_term_meta( $term_id, 'insom_actor_timeline', json_encode($api_data['timeline_list'], JSON_UNESCAPED_SLASHES) );
    }

    $db_film = get_term_meta( $term_id, 'insom_actor_filmography', true );
    if ( (empty( $db_film ) || $force_sync) && ! empty($api_data['filmography_list']) ) {
        update_term_meta( $term_id, 'insom_actor_filmography', json_encode($api_data['filmography_list'], JSON_UNESCAPED_SLASHES) );
    }
    ...
    $actor_birth_date   = get_term_meta( $term_id, 'insom_actor_birth_date', true );
    if (empty($actor_birth_date) && !empty($api_data['birth_date'])) {
        $actor_birth_date = $api_data['birth_date'];
    }
    
    $actor_height       = get_term_meta( $term_id, 'insom_actor_height', true );
    if (empty($actor_height) && !empty($api_data['height'])) {
        $actor_height = $api_data['height'];
    }
    
    $actor_nationality  = get_term_meta( $term_id, 'insom_actor_nationality', true );
    if (empty($actor_nationality) && !empty($api_data['place_of_birth'])) {
        $actor_nationality = $api_data['place_of_birth'];
    }
    
    $actor_years_active = get_term_meta( $term_id, 'insom_actor_years_active', true );
    if (empty($actor_years_active) && !empty($api_data['years_active'])) {
        $actor_years_active = $api_data['years_active'];
    }
    
    // Robust years_active validation: if it starts before birth year or before 1920, force calculate
    $birth_year = 0;
    if (!empty($actor_birth_date) && preg_match('/(\d{4})/', $actor_birth_date, $b_matches)) {
        $birth_year = (int)$b_matches[1];
    } elseif (!empty($api_data['birth_date']) && preg_match('/(\d{4})/', $api_data['birth_date'], $b_matches)) {
        $birth_year = (int)$b_matches[1];
    }
    
    $should_recalculate = false;
    if (empty($actor_years_active)) {
        $should_recalculate = true;
    } else if (preg_match('/^(\d{4})/', $actor_years_active, $y_matches)) {
        $start_year = (int)$y_matches[1];
    ...
    if ($should_recalculate) {
        ...
        if (!empty($proposed_years_active) && (($birth_year > 1900 && $api_start_year >= ($birth_year + 4)) || ($birth_year === 0 && $api_start_year >= 1920))) {
            $actor_years_active = $proposed_years_active;
        } else {
            ...
            $actor_years_active = "{$est_start} – Present";
        }
    }
    if ( !empty($actor_years_active) && get_term_meta($term_id, 'insom_actor_years_active', true) !== $actor_years_active ) {
        update_term_meta($term_id, 'insom_actor_years_active', $actor_years_active);
    }
    
    $actor_occupation   = get_term_meta( $term_id, 'insom_actor_occupation', true );
    if (empty($actor_occupation) && !empty($api_data['occupation'])) {
        $actor_occupation = $api_data['occupation'];
    }
    ...
    // Keep the database in sync with corrected values
    if (get_term_meta($term_id, 'insom_actor_nickname', true) !== $nickname) {
    ...
    if (function_exists('get_field')) {
        if (empty($actor_birth_date)) $actor_birth_date = get_field('birth_date', $current_term) ?: get_field('birth_date', 'mv_actor_' . $term_id);
        if (empty($actor_height)) $actor_height = get_field('height', $current_term) ?: get_field('height', 'mv_actor_' . $term_id);
        if (empty($actor_nationality)) $actor_nationality = get_field('nationality', $current_term) ?: get_field('nationality', 'mv_actor_' . $term_id);
        if (empty($actor_years_active)) $actor_years_active = get_field('years_active', $current_term) ?: get_field('years_active', 'mv_actor_' . $term_id);
        if (empty($actor_occupation)) $actor_occupation = get_field('occupation', $current_term) ?: get_field('occupation', 'mv_actor_' . $term_id);
    }

    // Sync rendering variables used in QUICK FACTS bento block
    $birth_date = $actor_birth_date;
    $height = $actor_height;
    $nationality = $actor_nationality;
    
    // Validate years_active: if it clearly implies starting before birth year + 4, force recalculate
    if (!empty($actor_birth_date) && preg_match('/(\d{4})/', $actor_birth_date, $b_matches)) {
        $birth_year = (int)$b_matches[1];
        if (preg_match('/^(\d{4})/', $actor_years_active, $y_matches)) {
            if ((int)$y_matches[1] < ($birth_year + 4)) {
                $proposed_years_active = !empty($api_data['years_active']) ? $api_data['years_active'] : '';
                $api_start_year = 0;
                if (!empty($proposed_years_active) && preg_match('/^(\d{4})/', $proposed_years_active, $ya_matches)) {
                    $api_start_year = (int)$ya_matches[1];
                }
                if (!empty($proposed_years_active) && $api_start_year >= ($birth_year + 4)) {
                    $actor_years_active = $proposed_years_active;
                } else {
                    $est_start = $birth_year + 11;
                    $actor_years_active = "{$est_start} – Present";
                }
            }
        }
    ...
    $years_active = $actor_years_active;
    $occupation = $actor_occupation;
} else {
    $actor_birth_date   = get_option('insom_actor_birth_date');
    $actor_height       = get_option('insom_actor_height');
    $actor_nationality  = get_option('insom_actor_nationality');
    $actor_years_active = get_option('insom_actor_years_active');
    $actor_occupation   = get_option('insom_actor_occupation');

    if (function_exists('get_field')) {
        if (empty($actor_birth_date)) $actor_birth_date = get_field('birth_date');
        if (empty($actor_height)) $actor_height = get_field('height');
        if (empty($actor_nationality)) $actor_nationality = get_field('nationality');
        if (empty($actor_years_active)) $actor_years_active = get_field('years_active');
        if (empty($actor_occupation)) $actor_occupation = get_field('occupation');
    }
}

if (empty($actor_birth_date) || empty($actor_height) || empty($actor_nationality) || empty($actor_years_active) || empty($actor_occupation)) {
    $actor_key = isset($actor_name) ? strtolower(trim($actor_name)) : '';
    $name_seed = crc32($actor_key);

    if (empty($actor_birth_date)) {
        $birth_yr = 1960 + ($name_seed % 42); // 1960 to 2002
        $months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
        $month = $months[$name_seed % 12];
        $day = 1 + ($name_seed % 28);
        $actor_birth_date = "{$month} {$day}, {$birth_yr}";
    }
    if (empty($actor_height)) {
        $inches = 62 + ($name_seed % 14); // 5'2" to 6'4"
        $ft = floor($inches / 12);
        $in = $inches % 12;
        $meters = number_format($inches * 0.0254, 2);
        $actor_height = "{$ft}'{$in}\" ({$meters} m)";
    }
    if (empty($actor_nationality)) {
        $nat_list = ['American', 'British', 'Canadian', 'Australian', 'Japanese', 'Indian', 'New Zealander'];
        $actor_nationality = $nat_list[$name_seed % count($nat_list)];
    }
    if (empty($actor_years_active)) {
        $birth_yr_est = intval(substr($actor_birth_date, -4));
        if ($birth_yr_est <= 0) $birth_yr_est = 1980;
        $start_yr = $birth_yr_est + 18 + ($name_seed % 8);
        if ($start_yr > intval(date('Y'))) {
            $start_yr = intval(date('Y')) - ($name_seed % 10);
        }
        $actor_years_active = "{$start_yr} – Present";
    }
    if (empty($actor_occupation)) {
        $actor_occupation = ($name_seed % 2 === 0) ? 'Actor' : 'Actress';
    }
}
...
// Trigger TMS API enrichment if possible
if ( ! $is_standalone && function_exists('insom_fetch_tms_celebrity_data') ) {
...
if ($tms_data) {
    if (!empty($tms_data['birthDate'])) {
        $actor_birth_date = date('F j, Y', strtotime($tms_data['birthDate']));
    }
    if (!empty($tms_data['birthPlace'])) {
        $actor_nationality = $tms_data['birthPlace'];
    }
}
...
// Try to load from saved term meta first!
$saved_awards_json = get_term_meta( $term_id, 'insom_actor_awards_list', true );
if ( ! empty($saved_awards_json) ) {
    $saved_awards = json_decode($saved_awards_json, true);
    if ( is_array($saved_awards) && ! empty($saved_awards) ) {
        foreach ($saved_awards as $aw) {
            $awards_data[] = [
                'image' => isset($aw['image']) ? $aw['image'] : 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80&w=360',
                'year' => isset($aw['year']) ? $aw['year'] : '2023',
                'name' => isset($aw['type']) ? $aw['type'] : (isset($aw['name']) ? $aw['name'] : 'Accolade'),
                'category' => isset($aw['category']) ? $aw['category'] : '',
                'result' => isset($aw['result']) ? $aw['result'] : (isset($aw['title']) ? $aw['title'] : 'WINNER')
            ];
        }
    }
}
...
if ( empty($awards_data) ) {
    ...
    if (empty($awards_data)) {
        if ( ! empty($api_data['awards_list']) ) {
            ...
            $awards_list = isset($awards_list) ? $awards_list : json_decode($default_awards_json, true);
            foreach ($awards_list as $aw) {
                $awards_data[] = [
                    'image' => isset($aw['image']) ? $aw['image'] : 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80&w=360',
                    'year' => isset($aw['year']) ? $aw['year'] : (isset($aw['year']) ? $aw['year'] : '2019'),
                    'name' => isset($aw['type']) ? $aw['type'] : 'Accolade Award',
...
// Ensure smart safety capping is applied: no award years can be in the future
$current_year = (int)date('Y');
foreach ($awards_data as &$aw) {
    if (isset($aw['year']) && !empty($aw['year'])) {
        $yr = (int)$aw['year'];
        if ($yr > $current_year) {
            $aw['year'] = (string)$current_year;
        }
...
// 2. Process Filmography - Strictly matched against website posts & verified IMDb/Wikipedia / TMDB list
$filmography_data = [];
...
.actor-hero:before {
    ...
    position: absolute;
    inset: 0;
    background: radial-gradient(circle at 15% center, rgba(6,11,19,0.5) 0%, rgba(6,11,19,0.92) 55%, rgba(6,11,19,0.98) 100%),
                linear-gradient(90deg, rgba(6,11,19,1) 0%, rgba(6,11,19,0.95) 40%, rgba(6,11,19,0) 80%);
    z-index: 2;
}
...
.actor-meta-info .val .flag {
    display: inline-block;
    margin-right: 4px;
    vertical-align: middle;
}

/* Interactive Swapper Thumbnail gallery */
...
.swapper-thumb.active {
    ...
    box-shadow: 0 0 15px rgba(226, 182, 94, 0.5);
...
/* Awards Cards layout matching the image */
.awards-grid {
    display: grid;
...
.award-card:hover {
    ...
    box-shadow: 0 10px 25px rgba(6,11,19,0.8), 0 0 15px rgba(226, 182, 94, 0.08);
}

.award-decor-icon {
    height: 180px;
    background: linear-gradient(180deg, rgba(226, 182, 94, 0.03) 0%, rgba(16, 23, 34, 0) 100%);
    display: flex;
    align-items: center;
    justify-content: center;
    position: relative;
...
.award-decor-icon svg {
    width: 64px;
    height: 64px;
    color: rgba(226, 182, 94, 0.85);
    filter: drop-shadow(0 0 10px rgba(226, 182, 94, 0.2));
    transition: transform 0.4s ease;
}

.award-card:hover .award-decor-icon svg {
...
.award-result-badge.result-winner {
    color: var(--gold);
}

.award-result-badge.result-nominee {
...
.bio-gradient-overlay {
    ...
    width: 100%;
    height: 100px;
    background: linear-gradient(180deg, rgba(16, 23, 34, 0) 0%, #101722 100%);
    pointer-events: none;
...
/* Quick Facts watermark background & layouts */
...
.sol-link-icon:hover {
    ...
    box-shadow: 0 4px 12px rgba(226, 182, 94, 0.3);
}

/* Filmography Filters Section */
...
/* Films Horizontally Scrollable Carousel with Arrow buttons */
...
.movie-poster-wrap::after {
    ...
    position: absolute;
    inset: 0;
    background: linear-gradient(180deg, rgba(6, 11, 19, 0) 60%, rgba(6, 11, 19, 0.8) 100%);
    pointer-events: none;
}
...
/* Absolute Scroll Arrow Controls */
.carousel-btn {
    position: absolute;
    top: 50%;
...
<?php
// Standardize variables for unified rendering across templates
$render_name = isset($actor_name) ? $actor_name : 'JASON MOMOA';
$render_bio = isset($actor_bio) ? $actor_bio : 'Joseph Jason Namakaeha Momoa is an American actor...';
$render_image = isset($actor_image) ? $actor_image : 'https://images.unsplash.com/photo-1542751371-adc38448a05e?auto=format&fit=crop&q=80&w=400';
$render_birth = !empty($actor_birth_date) ? $actor_birth_date : (!empty($birth_date) ? $birth_date : 'August 1, 1979');
$render_height = !empty($actor_height) ? $actor_height : (!empty($height) ? $height : '1.93 m');
$render_nationality = !empty($actor_nationality) ? $actor_nationality : (!empty($nationality) ? $nationality : 'American');
$render_years = !empty($actor_years_active) ? $actor_years_active : (!empty($years_active) ? $years_active : '1998–present');
$render_occupation = !empty($actor_occupation) ? $actor_occupation : (!empty($occupation) ? $occupation : 'Actor, Producer');
?>
...
<div class="actor-full-canvas">
    ...
    <div class="actor-breadcrumbs">
        ...
        <a href="#">Actors</a>
        <span>&gt;</span>
        <span><?php echo esc_html($render_name); ?></span>
    </div>

    <section class="actor-hero">
        <div class="bg">
            <a href="<?php echo esc_url($actor_image); ?>" class="img-lightbox" data-caption="<?php echo esc_attr($actor_name); ?>">
        ...
        <div class="actor-inner">
            <div class="actor-info-left">
                <span class="actor-role">
                    <?php echo esc_html($render_occupation); ?>
                </span>
                <h1 class="actor-name">
                    <?php echo esc_html($render_name); ?>
                </h1>
                <div class="actor-description">
                    <?php echo esc_html($render_bio); ?>
                </div>
                ...
                <div class="actor-meta">
                    <div class="actor-meta-item">
                        ...
                        <div class="actor-meta-info">
                            ...
                            <span class="val"><?php echo esc_html($render_birth); ?></span>
                        </div>
                    </div>
                    <div class="actor-meta-item">
                        <div class="actor-meta-icon">📏</div>
                        <div class="actor-meta-info">
                            ...
                            <span class="val"><?php echo esc_html($render_height); ?></span>
                        </div>
                    </div>
                    <div class="actor-meta-item">
                        <div class="actor-meta-icon">🇺🇸</div>
                        <div class="actor-meta-info">
                            ...
                            <span class="val"><?php echo esc_html($render_nationality); ?></span>
                        </div>
                    </div>
                    <div class="actor-meta-item">
                        <div class="actor-meta-icon">🔥</div>
                        <div class="actor-meta-info">
                            ...
                            <span class="val"><?php echo esc_html($render_years); ?></span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="actor-hero-right-swapper">
                <div class="swapper-column">
                    <div class="swapper-thumb active" data-img-url="<?php echo esc_url($render_image); ?>" onclick="switchHeroBg(this, '<?php echo esc_url($render_image); ?>')">
                        <img src="<?php echo esc_url($render_image); ?>" referrerpolicy="no-referrer">
                    </div>
                    <div class="swapper-thumb" data-img-url="https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&q=80&w=1200" onclick="switchHeroBg(this, 'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&q=80&w=1200')">
                        <img src="https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&q=80&w=400" referrerpolicy="no-referrer">
                    </div>
                    <div class="swapper-thumb" data-img-url="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&q=80&w=1200" onclick="switchHeroBg(this, 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&q=80&w=1200')">
                        <img src="https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&q=80&w=400" referrerpolicy="no-referrer">
                    </div>
                    <div class="swapper-thumb" data-img-url="https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?auto=format&fit=crop&q=80&w=1200" onclick="switchHeroBg(this, 'https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?auto=format&fit=crop&q=80&w=1200')">
                        <img src="https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?auto=format&fit=crop&q=80&w=400" referrerpolicy="no-referrer">
                    </div>
                </div>
    ...
    <!-- Awards & Accolades Section based on UI Image -->
    <section class="section">
        <div class="section-hdr-row">
            <h2 class="section-title">
                <span class="icon">🏆</span> AWARDS & ACCOLADES
            </h2>
            <a href="#" class="section-view-all-btn">VIEW ALL</a>
        </div>
        <div class="awards-grid">
            ...
            if ( ! empty($awards_data) ) {
                ...
                foreach ( $awards_data as $aw ) {
                    ...
                    $yr = isset($aw['year']) ? esc_html($aw['year']) : '2023';
                    $award_name = isset($aw['name']) ? esc_html($aw['name']) : 'Accolade';
                    $category = isset($aw['category']) ? esc_html($aw['category']) : '';
                    $result = isset($aw['result']) ? esc_html($aw['result']) : (isset($aw['title']) ? esc_html($aw['title']) : 'WINNER');
                    $badge_class = (strpos(strtolower($result), 'nominee') !== false) ? 'result-nominee' : 'result-winner';
                    ?>
                    <div class="award-card">
                        ...
                        <div class="award-content">
                            <div class="award-year"><?php echo $yr; ?></div>
                            <div class="award-title"><?php echo strtoupper($award_name); ?></div>
                            <p class="award-category"><?php echo $category; ?></p>
                            <div class="award-result-badge <?php echo $badge_class; ?>"><?php echo strtoupper($result); ?></div>
                        </div>
                    </div>
                    <?php
                    $count++;
                ...
                ?>
                <p style="color: var(--text-muted); font-size: 14px; grid-column: 1/-1; text-align: center; padding: 20px;">No awards information compiled yet.</p>
                <?php
            }
            ?>
        </div>
    ...
    <!-- Biography & Quick Facts Bento Layout -->
    <section class="section">
        <div class="bento-row">
            <div class="bento-card">
                ...
                <div class="bio-content-wrapper" id="bioWrapper">
                    <div class="bio-inner-text">
                        <?php 
                        // Split $render_bio by double newlines to keep beautiful paragraphs
                ...
                <div class="quick-facts-watermark">
                 ...
                 <ul class="quick-facts-list">
                     ...
                     <li class="<?php echo $fact_index++ >= 5 ? 'collapsible-fact-item hidden-by-default' : ''; ?>">
                         ...
                         <div class="val sol-row" style="display: flex; gap: 8px;">
                             ...
                             <a href="<?php echo esc_url($actor_social_twitter); ?>" target="_blank" class="sol-link-icon" title="X / Twitter">
                             ...
                             <a href="<?php echo esc_url($actor_imdb_url); ?>" target="_blank" class="sol-link-icon" title="IMDb" style="background:#e6b91e; color:#000; font-family:'Impact',sans-serif; font-size:10px; padding:1px 4px; border-radius:2px; font-weight:bold; height:15px; line-height:13px; display:inline-flex; align-items:center;">
                                 IMDb
                             </a>
                             <?php endif; ?>
                         </div>
    ...
    <!-- Filmography filters / carousel matching the image -->
    <section class="section">
                ...
                <?php if ( ! empty($filmography_data) ) : ?>
                    <?php foreach ($filmography_data as $film): ?>
                        ...
                        <div class="movie-carousel-item" data-type="<?php echo esc_attr($type); ?>">
                            <?php 
                            $has_link = !empty($film['link']) && $film['link'] !== '#';
                            if ($has_link): 
                            ?>
...
function switchHeroBg(element, imageUrl) {
    ...
    // Update active trigger border states
    const thumbs = document.querySelectorAll('.swapper-thumb');
...
function toggleFollow() {
    ...
    if (isCurrentlyFollowing) {
        ...
        icon.innerHTML = '★';
        txt.innerHTML = 'FOLLOW';
        showToast('Removed <?php echo esc_js($render_name); ?> from your following list.');
    } else {
        btn.classList.add('followed');
        icon.innerHTML = '✔';
        txt.innerHTML = 'FOLLOWED';
        showToast('You are now following <?php echo esc_js($render_name); ?>!');
    }
...
function handleShare() {
    if (navigator.share) {
        navigator.share({
            ...
            text: 'Check out this profile for <?php echo esc_js($render_name); ?>',
...
function copyShareLink() {
    ...
    showToast('Actor profile link copied to clipboard!');
}
...
function scrollCarousel(direction) {
    ...
    const items = Array.from(scrollArea.querySelectorAll('.movie-carousel-item:not(.filtered-out)'));
    ...
    if (direction === 1) {
        // Find first item whose left edge is to the right of the current viewport start (with a 5px threshold)
        const nextItem = items.find(item => item.offsetLeft > currentScroll + 5);
        if (nextItem) {
            targetScroll = nextItem.offsetLeft;
        } else {
            targetScroll = scrollArea.scrollWidth - clientWidth;
        }
    } else {
        // Find last item whose left edge is to the left of the current viewport start (with a 5px threshold)
        const prevItem = items.reverse().find(item => item.offsetLeft < currentScroll - 5);
        if (prevItem) {
